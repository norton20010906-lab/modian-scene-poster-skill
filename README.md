...
一个在 Codex 等支持视觉理解和内置图像生成的 Agent 环境中运行的 V1 Skill。它把产品图片、产品型号和可选补充需求转换成四张 1080×1350 中文使用场景海报候选和一张四宫格联系表；其中前三张来自成熟模板库，第四张是本轮新探索，用户选择后再交付单张成品。
...
- P1（左上）、P2（右上）、P3（左下）由 Skill 从内部成熟模板库按产品与场景匹配。
- P4（右下）每轮探索一种新排版，不从模板库直接套用。
- 四宫格只用于比较；用户回复 1～4 后展示对应的全尺寸海报。
- 用户明确认可 P4 并要求加入模板库时，Skill 会把它抽象成无具体产品文案的版式配方，验证后纳入未来 P1～P3 的轮换池。
...
- `SKILL.md`：Agent 执行入口。
- `data/`：品牌和单型号可信资料。
- `data/layout_templates.yaml`：成熟版式积累库、3+1 候选策略与晋升记录。
- `references/template_promotion_workflow.md`：把优秀 P4 抽象、验证并晋升为可复用模板的方法。
- `scripts/promote_layout_template.py`：经明确授权后生成待审核的新模板注册表。
